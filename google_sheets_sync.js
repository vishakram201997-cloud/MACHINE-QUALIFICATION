...
(function() {
    ...
    function saveOverrideToGoogleSheets(key, value) {
        ...
        fetch(window.googleSheetsSyncUrl, {
            ...
            mode: 'cors',
            headers: {
                'Content-Type': 'text/plain' // Simple request to bypass CORS preflight OPTIONS request
            },
            body: JSON.stringify({
        ...
        .then(res => res.json())
        .then(data => {
            console.log(`[Google Sync] Backed up ${key} to Google Sheets. Status: ${data.status}`);
        })
        .catch(err => {
